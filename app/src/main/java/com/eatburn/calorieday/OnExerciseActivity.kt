...
import com.eatburn.calorieday.R.layout.example_item2
...
class OnExerciseActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_on_track2)
        mAuth = FirebaseAuth.getInstance()
        mAuthListener = FirebaseAuth.AuthStateListener { firebaseAuth ->
            val users = firebaseAuth.currentUser
            if (users == null) {
                startActivity(Intent(this@OnExerciseActivity, LoginActivity::class.java))
                finish()
            }
        }
...
//        FirebaseDatabase.getInstance().reference.child("Diary_Logs")
        ...
        mQueryCurrentUser = mDatabase!!.child("Exercise").orderByChild("Exercise")
    ...
    private fun deleteUserData(mKey: String) {
        ...
        FirebaseDatabase.getInstance().reference.child(mAuth!!.currentUser!!.uid).child("Exercise").child(mKey).
        addListenerForSingleValueEvent(object : ValueEventListener {
            ...
            override fun onDataChange(p0: DataSnapshot) {
                ...
                FirebaseDatabase.getInstance().reference.child(mAuth!!.currentUser!!.uid).child("Exercise").child(mKey).removeValue() // It's Work
    ...
    override fun onStart() {
        ...
        val firebaseRecyclerAdapter: FirebaseRecyclerAdapter<ExampleItem, ExampleAdapter> =
            object : FirebaseRecyclerAdapter<ExampleItem, ExampleAdapter>(
                ...
                override fun onBindViewHolder(viewHolder: ExampleAdapter, position: Int) {
                    super.onBindViewHolder(viewHolder, position)
                    viewHolder.itemView.setOnClickListener {
                        // Create Layout in Dialog
                        val layout = LinearLayout(this@OnExerciseActivity)
                        layout.setOrientation(LinearLayout.VERTICAL)
                        // Create EditText and TextView
                        val firstval : EditText = EditText(this@OnExerciseActivity)
                        val secondval : EditText = EditText(this@OnExerciseActivity)
                        val readtext1 : TextView = MaterialTextView(this@OnExerciseActivity)
                        val readtext2 : TextView = MaterialTextView(this@OnExerciseActivity)
                        ...
                        val builder = AlertDialog.Builder(this@OnExerciseActivity)
                        // Set the alert dialog title
                        secondval.setInputType(InputType.TYPE_CLASS_NUMBER)
                        builder.setTitle(R.string.update)
                        firstval.setHint(getString(R.string.activity));
                        secondval.setHint(R.string.calories)
                        readtext1.setText(getString(R.string.pre_act))
                        readtext1.append(": " + Menu)
                        readtext2.setText(R.string.pre_cal)
                        ...
                        builder.setPositiveButton(R.string.update) { dialog, which ->
                            FirebaseDatabase.getInstance().reference.child(mAuth!!.currentUser!!.uid).child("Exercise")
                        ...
                        builder.setNegativeButton(R.string.cancel){ dialog, which ->
                            Toast.makeText(applicationContext,
                                "Nothing Update",Toast.LENGTH_SHORT).show()
                        }
                        // Display a message on alert dialog
                        builder.setMessage(R.string.Input_update)
                    ...
                    viewHolder.itemView.setOnLongClickListener {
                        val FirebaseKey = (mEntryList!!.findViewHolderForAdapterPosition(position)!!
                            .itemView.findViewById(R.id.mKey) as TextView).text.toString()
                        var img: String? = (mEntryList!!.
                        findViewHolderForAdapterPosition(position)!!
                            .itemView.findViewById(R.id.mImage) as TextView).text.toString()
                        Log.d("mKey",FirebaseKey)
                        Log.d("Position", viewHolder.adapterPosition.toString())
                        Log.d("mImage",img!!)
                        if (img == null)
                        {
                            Log.d("mImage", "null image")
                        }
                        val builder = AlertDialog.Builder(this@OnExerciseActivity)
                        builder.setTitle(getString(R.string.delete_ex))
                        // Display a message on alert dialog
                        builder.setMessage(getString(R.string.delete_pop_ex))
...
//                                deleteimg(img)
                            ...
                            notifyItemRangeChanged(viewHolder.adapterPosition,mEntryList!!.size)
